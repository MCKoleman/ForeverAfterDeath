...
[CreateAssetMenu(fileName = "LevelRoomList", menuName = "ScriptableObjects/LevelRoomList", order = 1)]
[System.Serializable]
public class LevelRoomList : ScriptableObject
{
    [SerializeField]
    ...
    public WeightedGameObjectList GetRoomsByReq(uint reqFlag, WeightedGameObjectList rooms)
    {
        ...
        for (int i = 0; i < rooms.Count(); i++)
        {
            ...
            LevelRoom tempRoom = rooms.GetObject(i).GetComponent<LevelRoom>();
            if (tempRoom == null)
                continue;
